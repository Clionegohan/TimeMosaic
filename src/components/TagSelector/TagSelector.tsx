  ...
  if (variant === 'header') {
    return (
      <div className="min-w-0 flex items-center gap-3">
        <div className="text-sm font-medium text-stone-700 whitespace-nowrap">タグで絞り込む</div>
        ...
        >
          <div className="flex items-center gap-2 flex-nowrap py-1">
            {selectedTags.length === 0 ? (
              <span className="text-sm text-stone-500 whitespace-nowrap">未選択（クリックで追加）</span>
            ) : (
      ...
      {isMaxReached && (
        <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl">
          <p className="text-sm text-amber-900">
            最大{MAX_COLUMNS}列まで選択可能です。別のタグを選択する場合は、現在選択中のタグを削除してください。
          </p>
        </div>
      )}
